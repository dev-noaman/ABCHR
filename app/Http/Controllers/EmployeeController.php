...
class EmployeeController extends Controller
{
    ...
    public function downloadTemplate()
    {
        ...
        $columns = [
            'name', 'email', 'password', 'phone', 'personal_number',
            'date_of_birth', 'gender', 'branch_id', 'department_id', 'designation_id',
            'date_of_joining', 'employment_type', 'employee_status',
        ...
        $sampleRow = [
            'John Doe', 'john.doe@example.com', 'password123', '97433445566', '97433445577',
            '1990-01-15', 'male', '1', '1', '1',
            '2024-01-01', 'Full-time', 'active',
            '123 Main St', 'Doha', 'Qatar',
            'Jane Doe', '97433445588',
            'Qatar National Bank', 'John Doe', 'QA12345678',
        ];
    ...
    public function bulkImport(Request $request)
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if (User::where('email', trim($data['email']))->exists()) {
                $failed++;
                $errors[] = "Row {$rowNum}: email '{$data['email']}' already exists";
                continue;
            }
            ...
            try {
                ...
                $user->password = Hash::make(!empty($data['password']) ? trim($data['password']) : 'password');
                $user->type = 'employee';
                $user->lang = 'en';
                $user->created_by = creatorId();
                $user->save();
                ...
                $employee->user_id = $user->id;
                $employee->employee_id = Employee::generateEmployeeId();
                $employee->phone = !empty($data['phone']) ? trim($data['phone']) : null;
                $employee->personal_number = !empty($data['personal_number']) ? trim($data['personal_number']) : null;
                $employee->date_of_birth = !empty($data['date_of_birth']) ? trim($data['date_of_birth']) : null;
                $employee->gender = !empty($data['gender']) ? trim($data['gender']) : null;
                $employee->branch_id = !empty($data['branch_id']) ? (int)$data['branch_id'] : null;
                $employee->department_id = !empty($data['department_id']) ? (int)$data['department_id'] : null;
                $employee->designation_id = !empty($data['designation_id']) ? (int)$data['designation_id'] : null;
                $employee->date_of_joining = !empty($data['date_of_joining']) ? trim($data['date_of_joining']) : now()->toDateString();
                $employee->employment_type = !empty($data['employment_type']) ? trim($data['employment_type']) : 'Full-time';
                $employee->employee_status = !empty($data['employee_status']) ? trim($data['employee_status']) : 'active';
                $employee->address_line_1 = !empty($data['address_line_1']) ? trim($data['address_line_1']) : null;
                $employee->city = !empty($data['city']) ? trim($data['city']) : null;
                $employee->country = !empty($data['country']) ? trim($data['country']) : null;
                $employee->emergency_contact_name = !empty($data['emergency_contact_name']) ? trim($data['emergency_contact_name']) : null;
                $employee->emergency_contact_number = !empty($data['emergency_contact_number']) ? trim($data['emergency_contact_number']) : null;
                $employee->bank_name = !empty($data['bank_name']) ? trim($data['bank_name']) : null;
                $employee->account_holder_name = !empty($data['account_holder_name']) ? trim($data['account_holder_name']) : null;
                $employee->account_number = !empty($data['account_number']) ? trim($data['account_number']) : null;
                $employee->created_by = creatorId();
                $employee->save();
        ...
        $sessionData = ['imported' => $imported, 'failed' => $failed, 'errors' => $errors, 'message' => $message];
