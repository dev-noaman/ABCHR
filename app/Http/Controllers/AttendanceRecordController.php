...
class AttendanceRecordController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-attendance-records')) {
            ...
            // Handle outside location filter
            if ($request->has('outside_location') && $request->outside_location !== '' && $request->outside_location !== 'all') {
                $flag = (bool) $request->outside_location;
                $query->where(function ($q) use ($flag) {
                    $q->where('clock_in_outside_location', $flag)
                      ->orWhere('clock_out_outside_location', $flag);
                });
            }
            ...
            return Inertia::render('hr/attendance-records/index', [
                ...
                'filters' => $request->all(['search', 'employee_id', 'status', 'date_from', 'date_to', 'outside_location', 'sort_field', 'sort_direction', 'per_page']),
            ]);
        } else {
    ...
    /**
     * Check location restriction and return whether the punch is outside all allowed locations.
     * Returns null if restriction is disabled. Returns an error redirect string key if coords missing or no locations configured.
     */
    private function checkLocationRestriction(array $validated, array $settings): bool|string
    {
        if (empty($settings['locationRestrictionEnabled']) || $settings['locationRestrictionEnabled'] != 1) {
            return false;
        }
        ...
        $locationBinds = LocationBind::whereIn('created_by', getCompanyAndUsersId())->get();
        if ($locationBinds->isEmpty()) {
            return 'no_locations';
        }

        foreach ($locationBinds as $location) {
            if (isWithinLocationRadius(
                ...
                return false; // inside a location
            }
        }

        return true; // outside all locations
    }
    ...
    public function clockIn(Request $request)
    {
        if (Auth::user()->can('clock-in-out')) {
            try {
                ...
                $locationCheck = $this->checkLocationRestriction($validated, $settings);
                if ($locationCheck === 'missing_coords') {
                    return redirect()->back()->with('error', __('Location is required to clock in. Please enable location access.'));
                }
                if ($locationCheck === 'no_locations') {
                    return redirect()->back()->with('error', __('No clock locations configured. Please contact admin.'));
                }
                $clockInOutsideLocation = $locationCheck === true;
                ...
                $clockInData = [
                    ...
                    'clock_in_outside_location' => $clockInOutsideLocation,
                ];

                if ($existingRecord) {
                    $existingRecord->update($clockInData);
                    $record = $existingRecord;
                } else {
                    $record = AttendanceRecord::create(array_merge($clockInData, [
                ...
                $response = redirect()->back()->with('success', __('Clocked in successfully.'));
                if ($clockInOutsideLocation) {
                    $response = $response->with('clockedOutsideLocation', true);
                }
                return $response;
            } catch (\Exception $e) {
    ...
    public function clockOut(Request $request)
    {
        if (Auth::user()->can('clock-in-out')) {
            try {
                ...
                $locationCheck = $this->checkLocationRestriction($validated, $settings);
                if ($locationCheck === 'missing_coords') {
                    return redirect()->back()->with('error', __('Location is required to clock out. Please enable location access.'));
                }
                if ($locationCheck === 'no_locations') {
                    return redirect()->back()->with('error', __('No clock locations configured. Please contact admin.'));
                }
                $clockOutOutsideLocation = $locationCheck === true;
                ...
                $record->update([
                    ...
                    'clock_out_outside_location' => $clockOutOutsideLocation,
                ]);
                ...
                $response = redirect()->back()->with('success', __('Clocked out successfully.'));
                if ($clockOutOutsideLocation) {
                    $response = $response->with('clockedOutsideLocation', true);
                }
                return $response;
            } catch (\Exception $e) {
