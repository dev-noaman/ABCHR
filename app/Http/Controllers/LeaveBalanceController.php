...
class LeaveBalanceController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-leave-balances')) {
            // Exclude leave types with no balance tracking (sick, unpaid)
            $noBalanceSlugs = \App\Models\LeaveType::NO_BALANCE_SLUGS;

            $query = LeaveBalance::with(['employee', 'leaveType', 'leavePolicy', 'creator'])
                ->whereHas('leaveType', fn ($q) => $q->whereNotIn('slug', $noBalanceSlugs))
                ->where(function ($q) {
            ...
            // Get leave types for filter dropdown (exclude no-balance types)
            $leaveTypes = LeaveType::whereIn('created_by', getCompanyAndUsersId())
                ->where('status', 'active')
                ->whereNotIn('slug', \App\Models\LeaveType::NO_BALANCE_SLUGS)
                ->get(['id', 'name', 'color']);
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'prior_used_days' => 'nullable|numeric|min:0',
        ]);

        $validated['created_by']       = creatorId();
        $validated['prior_used_days']  = $validated['prior_used_days'] ?? 0;
        $validated['carried_forward']  = 0;
        $validated['manual_adjustment'] = 0;

        // Prevent creating balances for no-balance types
        $leaveType = LeaveType::find($validated['leave_type_id']);
        if ($leaveType && ! $leaveType->tracksBalance()) {
            return redirect()->back()->with('error', __(':type does not use balance tracking.', ['type' => $leaveType->name]));
        }
        ...
        $validated['remaining_days'] = $validated['allocated_days'] - $validated['prior_used_days'];
    ...
    public function update(Request $request, $leaveBalanceId)
    {
        ...
        if ($leaveBalance) {
            try {
                ...
                // Recalculate remaining days: allocated - prior_used - system_used
                $validated['remaining_days'] = $validated['allocated_days']
                    - $validated['prior_used_days']
                    - (float) $leaveBalance->used_days;
