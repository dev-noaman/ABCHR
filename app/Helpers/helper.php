...
if (!function_exists('isWithinLocationRadius')) {
    /**
     * Determine whether a user coordinate is within the allowed radius of a location.
     *
     * Uses the Haversine formula to compute the great-circle distance between two
     * GPS coordinates and compares it against the configured radius.
     *
     * @param  float  $userLat      User's current latitude
     * @param  float  $userLng      User's current longitude
     * @param  float  $locLat       Location's latitude
     * @param  float  $locLng       Location's longitude
     * @param  int    $radiusMeters Allowed radius in metres
     * @return bool
     */
    function isWithinLocationRadius(float $userLat, float $userLng, float $locLat, float $locLng, int $radiusMeters): bool
    {
        $earthRadiusMeters = 6371000;

        $latDelta = deg2rad($locLat - $userLat);
        $lngDelta = deg2rad($locLng - $userLng);
